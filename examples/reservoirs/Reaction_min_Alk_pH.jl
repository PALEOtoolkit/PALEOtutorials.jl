...
Minimal example, TA_conc is first order decay of a variable.
...
Base.@kwdef mutable struct Reaction_Alk_pH_01{P} <: PB.AbstractReaction
    ...
    pars::P = PB.ParametersTuple(
        ...
        PB.ParDouble("TA_conc_change",   10.0e-6,    units="mol kg-1",      description="TA concentration change every kappa"),
...
function PB.register_methods!(rj::Reaction_Alk_pH_01)
    vars = [
        ...
        PB.VarDep("TA",                  "mol",           "reservoir for species TA"),
        PB.VarDep("TA_conc",             "mol m-3",       "TA concentration"),                    
        PB.VarContrib("TA_sms",          "mol yr-1",      "reservoir TA source - sink"),
        PB.VarProp("TA_decay_flux",      "mol yr-1",      "decay flux from reservoir TA"),        
        PB.VarConstraint("TA_error",     "mol m-3",       "in order to solve TA, we set it"),
...
function do_Min_Alk_pH_01(m::PB.ReactionMethod, pars, (varsdata, ), cellrange::PB.AbstractCellRange, deltat)
    ...
    for i in cellrange.indices
        ...
        varsdata.TA_decay_flux[i] = pars.kappa[] * pars.TA_conc_change[] * pars.density[] * varsdata.volume[i]      

        #         mol yr-1                    mol yr-1            
        varsdata.TA_sms[i] = varsdata.TA_decay_flux[i] 
        ...
        # mol kg-1               mol m-3           kg m-3
        TA_conc_kg = varsdata.TA_conc[i] / pars.density[]
        ...
        #       mol kg-1       mol kg-1          mol kg-1       mol kg-1     mol kg-1     mol kg-1                                 
        TA_conc_kg_calcu = HCO3_conc_kg + 2 * CO3_conc_kg + BOH4_conc_kg + OH_conc_kg  - H_conc_kg  
        ...
        varsdata.TA_error[i] = varsdata.TA_conc[i] - TA_conc_kg_calcu * pars.density[]
