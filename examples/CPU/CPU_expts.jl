...
function CPU_expts(model, expts)
...
function plot_CPU(output; pager=PALEOmodel.DefaultPlotPager())
    ...
    pager(
        ...
        plot(output, "global.U_norm",                   title="Uranium", ylabel="U/U_0 (normalized)"),
        plot(output, "global.U_delta",                  title="Uranium isotopes", ylabel="delta 238U (per mil)"),
        plot(output, "global.A_delta",                  title="Carbon isotopes", ylabel="delta 13C (per mil)"),
