# In this file I have added the zooplankton reaction from line 81
...
function grow_zoo(m::PB.ReactionMethod, (vars, ), cellrange::PB.AbstractCellRange, deltat)
    ...
    @inbounds for i in cellrange.indices
        ...
        if (vars.PX_conc[i] > rj.pars.phyt_thresh.v) && (vars.O2_conc[i] > rj.pars.O2_thresh.v)
            vars.ZX_growth_rate[i] = max(vars.PX_conc[i] - rj.pars.phyt_thresh.v, 0.0)*(rj.pars.gamma.v*HoverK_yr)
        else
            vars.ZX_growth_rate[i] = 0.0         
        end
        
        phytoplankton_consumption_rate = max(vars.ZX_conc[i], 0.0)*vars.ZX_growth_rate[i]

        # here we are accounting for the fact that zooplankton have an imperfect assimilation efficiency of the plankton they consume 
        # the plankton nutrient they don't incorporate into their biomass (set by gamma) has to go back into the nutrient pool of the ocean
        vars.ZX_sms[i] += phytoplankton_consumption_rate 
        vars.PX_sms[i] -= phytoplankton_consumption_rate / rj.pars.gamma.v
        
        phytoplankton_nutrient_release = (phytoplankton_consumption_rate / rj.pars.gamma.v) - phytoplankton_consumption_rate
        vars.P_sms[i] += phytoplankton_nutrient_release
        vars.O2_sms[i] += phytoplankton_nutrient_release*rj.pars.stoich_PtoO2.v
