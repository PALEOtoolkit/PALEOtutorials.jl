...
Base.@kwdef mutable struct ReactionZooplankton{P} <: PB.AbstractReaction
    ...
    pars::P = PB.ParametersTuple(
        # Constants for the NPZ model (Armstrong 1994)
        PB.ParDouble("mumax", 0.7, units="d-1",
            description="max growth rate"),
        PB.ParDouble("gamma", 0.1, 
            description="fraction of phytoplankton prey biomass turned into zooplankton biomass"),
        PB.ParDouble("Ks", 1e-3, units="mol m-3",
            description="phytoplankton limitation factor"),
        PB.ParDouble("O2_thresh", 1e-6, units="mol m-3",
            description="oxygen threshold constant"),
        PB.ParDouble("stoich_PtoO2", -106.0, units="",
            description="stoichiometry, negative by convention"),
...
function PB.register_methods!(rj::ReactionZooplankton)
    ...
    vars = [
        ...
        # PB.VarDep("O2_conc",            "mol m-3",      "oxygen conc"),
        PB.VarContrib("O2_sms",         "mol yr-1",     "Oxygen source-sink"),
        PB.VarContrib("P_sms",          "mol yr-1",     "Phosphorus source-sink"),
        PB.VarDep("PX_conc",            "mol m-3",      "Phytoplankton conc"),
        PB.VarContrib("PX_sms",         "mol yr-1",     "Phytoplankton source-sink"),
        PB.VarProp("ZX_feeding_rate",   "yr-1",         "Zooplankton feeding rate"),
        PB.VarDep("ZX_conc",            "mol m-3",      "Zooplankton concentration"),
        PB.VarContrib("ZX_sms",         "mol yr-1",     "Zooplankton source-sink"),
    ]
...
function grow_zoo(m::PB.ReactionMethod, pars, (vars, ), cellrange::PB.AbstractCellRange, deltat)
    ...
    for i in cellrange.indices
        ...
        # specific feeding rate limited by phytoplankton concentration
        # yr-1
        vars.ZX_feeding_rate[i] = mumax_yr*max(vars.PX_conc[i], 0.0)/(max(vars.PX_conc[i], 0.0) + pars.Ks[])

        # convert to population feeding rate in this ocean cell
        # mol P yr-1 = yr-1               * mol m-3                 * m^3
        pop_feeding_rate = vars.ZX_feeding_rate[i]*max(vars.ZX_conc[i], 0.0)*vars.volume[i]

        # phytoplankton loss
        vars.PX_sms[i] -= pop_feeding_rate  

        # zooplankton growth
        vars.ZX_sms[i] += pars.gamma[]*pop_feeding_rate

        # phytoplankton not assimilated into new biomass is respired (consuming O2) and P excreted
        pop_resp_rate = (1.0 - pars.gamma[])*pop_feeding_rate
        vars.P_sms[i] += pop_resp_rate
        vars.O2_sms[i] += pop_resp_rate*pars.stoich_PtoO2[]       
    end
